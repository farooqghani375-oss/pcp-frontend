    ...
    try {
      const orderData = {
        ...
        items: cart.map(i => ({
          ...
          size: i.selectedSize
            ? `${i.selectedSize.size_label} ${i.selectedSize.unit}`
            : null,
        })),
        subtotal,
        shipping: SHIPPING,
  ...
  return (
    <div className="pt-14 max-w-lg mx-auto px-4 pb-6">
      ...
      <div className="flex flex-col gap-3 mb-6">
        {cart.map(item => {
          const itemKey = `${item.id}-${item.selectedColor}-${item.selectedSize?.size_label ?? 'none'}`
          return (
    ...
    <div className="w-full h-full flex items-center justify-center text-2xl">🌿</div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                  ...
                  {item.selectedSize && (
                    <span className="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded font-medium">
                      {item.selectedSize.size_label} {item.selectedSize.unit}
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  <button
                    onClick={() => updateQuantity(item.id, item.selectedColor, item.quantity - 1, item.selectedSize)}
                    className="w-6 h-6 rounded-full bg-gray-100 text-sm font-bold flex items-center justify-center"
                  >−</button>
                  <span className="text-sm font-semibold w-4 text-center">{item.quantity}</span>
                  <button
                    onClick={() => updateQuantity(item.id, item.selectedColor, item.quantity + 1, item.selectedSize)}
                    className="w-6 h-6 rounded-full bg-primary text-white text-sm font-bold flex items-center justify-center"
                  >+</button>
                </div>
              </div>
              ...
              <button
                ...
                className="text-gray-300 hover:text-red-400 text-xl self-start"
              >✕</button>
            </div>
          )
        })}
